...
The REST API documentation can be found on [docs.ceramic.ai](https://docs.ceramic.ai/). The full API of this library can be found in [api.md](api.md).
...
to add `CERAMIC_API_KEY="My API Key"` to your `.env` file
so that your API Key is not stored in source control.

## Async usage

Simply import `AsyncCeramic` instead of `Ceramic` and use `await` with each API call:
...
You can enable logging by setting the environment variable `CERAMIC_LOG` to `info`.

```shell
$ export CERAMIC_LOG=info
```
